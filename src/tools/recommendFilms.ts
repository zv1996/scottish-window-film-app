...
export const recommendFilms = {
  ...
  handler: async (input: any) => {
    ...
    if (!input.application) missing.push("application (e.g., living_room, office)");
    if (!input.vlt_preference) missing.push("brightness preference (brighter/neutral/darker)");
    if (!input.budget_level) missing.push("budget (value/mid/premium)");
    if (!input.install_location) missing.push("install location (interior/exterior)");
    if (!input.sun_exposure && !input.orientation) missing.push("sun exposure or orientation");
    ...
    const recs = scoreFilms(films as any[], { property_type, goals });
    return {
