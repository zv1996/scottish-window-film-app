...
export function scoreFilms(films, { property_type, goals }) {
    const filmsArr = normalizeFilms(films);
    if (!filmsArr.length)
        return [];
    const goalsLc = goals.map(g => String(g).toLowerCase());
    const scored = filmsArr.map((film) => {
        ...
        goalsLc.forEach(goal => { if (useCases.includes(goal)) {
            score += 0.4;
            why.push(`Good for ${goal}`);
        } });
        ...
        if (ir && goals.includes("heat"))
            score += ir / 500;
        if (uv && goals.includes("uv"))
            score += uv / 500;
        if (vlt && goals.includes("privacy") && vlt < 20)
            score += 0.2;
