...
const hasGoal = (goals, g) => Array.isArray(goals) && goals.map(x => String(x).toLowerCase()).includes(g.toLowerCase());
const parseMil = (name) => {
    if (!name)
        return null;
...
export function scoreFilms(films, { property_type, goals, vlt_preference, install_location, budget_level, }) {
    ...
    const baseScored = filmsArr.map((film) => {
        ...
        const useCases = Array.isArray(film.use_cases)
        ...
        if (install_location) {
            ...
            const isExteriorCapable = /exterior/i.test(categoryStr) || /osw|exterior/i.test(seriesStr) || /exterior/i.test(nameStr);
        ...
        if (budget_level) {
            ...
            if (tier && tierStr) {
                if (/value/i.test(tier) && /value|basic|good/i.test(tierStr))
                    score += 0.05;
                if (/mid/i.test(tier) && /mid|better|standard/i.test(tierStr))
                    score += 0.05;
                if (/premium/i.test(tier) && /premium|best|elite/i.test(tierStr))
                    score += 0.05;
            }
        }
    ...
    const sufficient = (top3.length >= 3 && maxScore >= 0.8) ||
    ...
    const broadened = baseScored.map((film) => {
        ...
        const vlt = Number(film.visible_light_transmission) || 0;
        const uv = Number(film.uv_rejection_pct) || 0;
        const looksSolar = /solar|sun\s*control|reflect/i.test(categoryStr) ||
    ...
    return unique.map(({ sku, brand, visible_light_transmission, ir_reduction_pct, uv_rejection_pct, score, why }) => ({
        sku,
        brand,
        vlt: visible_light_transmission,
        ir_pct: ir_reduction_pct,
