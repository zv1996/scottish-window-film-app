...
function vltScore(vlt, pref) {
    if (typeof vlt !== "number")
        return 0;
    const p = (pref || "").toLowerCase();
    if (p === "brighter")
        return vlt; // higher VLT is better
    if (p === "darker")
        return 100 - vlt; // lower VLT is better
    return Math.abs(50 - vlt) * -1; // "neutral": closer to 50 is better (less penalty)
}
function budgetTierRank(t) {
    const map = { premium: 3, mid: 2, value: 1 };
    return map[(t || "").toLowerCase()] || 0;
}
function budgetAffinity(t, wanted) {
    if (!wanted)
        return 0;
...
function whyMatchCount(film, wantGoals) {
    if (!wantGoals.size)
        return 0;
    const g = wantGoals;
    const uc = Array.isArray(film?.use_cases) ? film.use_cases.map((u) => String(u).toLowerCase()) : [];
    let c = 0;
    if ((g.has("heat") || g.has("glare")) && (film?.category === "solar_control" || uc.includes("heat") || uc.includes("glare")))
        c++;
    if (g.has("uv") && (uc.includes("uv") || film?.category === "solar_control"))
        c++;
    if (g.has("privacy") && (uc.includes("privacy") || film?.category === "privacy"))
        c++;
    if (g.has("security") && (uc.includes("security") || film?.category === "security"))
        c++;
    if (g.has("decorative") && (uc.includes("decorative") || film?.category === "decorative"))
        c++;
    return c;
}
function installLabel(film) {
    const locs = Array.isArray(film?.install_location) ? film.install_location.map((l) => l.toLowerCase()) : [];
    if (film?.exterior_ok === true || locs.includes("exterior"))
        return "Exterior‑capable";
    return "Interior";
}
const films = (() => {
    ...
    for (const block of brandBlocks) {
        ...
        for (const p of items) {
            const skuSynth = `${toKebab(brandName)}-${toKebab(p.product_name || p.series || "film")}`;
            out.push({
                sku: skuSynth,
                brand: brandName,
                ...
                visible_light_transmission: p.visible_light_transmission ??
                    p.vlt ??
                    undefined,
                ir_reduction_pct: p.ir_reduction_pct ??
                    p.ir_reduction ??
                    undefined,
                uv_rejection_pct: p.uv_rejection_pct ??
                    p.uv_rejection ??
                    undefined,
...
function expandToProducts(recs, catalog, goals, ctx) {
    ...
    const matchesGoal = (film) => {
        if (!wantGoals.size)
            return true;
        const uc = Array.isArray(film?.use_cases)
            ? film.use_cases.map((u) => String(u).toLowerCase())
            : [];
    ...
    const matchesContext = (film) => {
        if (wantProp) {
            const pts = Array.isArray(film?.best_for)
                ? film.best_for.map((p) => p.toLowerCase())
                : [];
            if (pts.length && !pts.includes(wantProp))
                return false;
        }
        if (wantLoc) {
            const locs = Array.isArray(film?.install_location)
                ? film.install_location.map((l) => l.toLowerCase())
                : [];
            if (locs.length && !locs.includes(wantLoc))
                return false;
            if (wantLoc === "exterior" && film?.exterior_ok === false)
                return false;
        }
        return true;
    };
    // Compute a composite score for each candidate to allow consistent sorting.
    const goalWeight = (film) => {
        let score = 0;
        // base boosts by goal/category
        if (film?.category === "solar_control" && (wantGoals.has("heat") || wantGoals.has("glare")))
            score += 4;
        if (wantGoals.has("uv") && (film?.category === "solar_control" || film?.use_cases?.includes?.("uv")))
            score += 2;
        if (wantGoals.has("privacy") && (film?.use_cases?.includes?.("privacy")))
            score += 3;
        if (wantGoals.has("security") && film?.category === "security")
            score += 4;
        if (wantGoals.has("decorative") && film?.category === "decorative")
            score += 3;
        // VLT preference
        const vlt = (typeof film?.visible_light_transmission === "number")
            ? film.visible_light_transmission
            : (typeof film?.vlt === "number" ? film.vlt : undefined);
    ...
    tierPreferred.sort((a, b) => {
        ...
        const av = typeof a?.visible_light_transmission === "number" ? a.visible_light_transmission : (typeof a?.vlt === "number" ? a.vlt : undefined);
        const bv = typeof b?.visible_light_transmission === "number" ? b.visible_light_transmission : (typeof b?.vlt === "number" ? b.vlt : undefined);
        const pref = wantVLT || "neutral";
        const clos = (v) => {
            if (typeof v !== "number")
                return 999;
            if (pref === "brighter")
                return Math.abs(100 - v);
            if (pref === "darker")
                return Math.abs(v - 0);
...
const recommendFilmsInputSchema = {
    property_type: z
        ...
        description: "Type of property. Affects which films are allowed and how we weight privacy vs heat rejection.",
    }),
    goals: z
        .array(z.enum([
        "heat",
        ...
        description: "Primary goals for the film. 'heat' = heat rejection, 'glare' = reduce glare, 'uv' = fade protection, 'privacy' = daytime privacy, 'security' = shatter resistance, 'decorative' = frosted/branding/etc.",
    }))
        .min(1, "Provide at least one goal like 'heat' or 'privacy'."),
    city: z
        .string({
        description: "Optional: city or region. Used only for climate/context in messaging.",
    })
        .optional(),
    sun_exposure: z
        .enum(["low", "medium", "high"], {
        description: "Optional: how intense the sun load is on these windows.",
    })
        .optional(),
    application: z
        .enum([
        ...
        description: "Where the film is going. Helps weight privacy vs aesthetic vs heat.",
    })
        .optional(),
    vlt_preference: z
        .enum(["brighter", "neutral", "darker"], {
        description: "Brightness preference from inside. 'brighter' = high VLT, 'darker' = tinted look / more privacy.",
    })
        .optional(),
    budget_level: z
        .enum(["value", "mid", "premium"], {
        description: "Rough budget tier. 'value' = cost-sensitive, 'premium' = flagship/lowest reflectivity/best performance.",
    })
        .optional(),
    install_location: z
        .enum(["interior", "exterior"], {
        description: "Mounting location. Some films are exterior-rated for commercial curtain wall.",
    })
        .optional(),
    orientation: z
        .enum(["north", "east", "south", "west"], {
        description: "Window orientation if known; helps approximate sun exposure.",
    })
        .optional(),
};
export const recommendFilms = {
    ...
    descriptor: {
        ...
        description: "Returns the top 5 film recommendations for this scenario. If critical context is missing (like brightness preference or application), asks clarifying questions instead of guessing.",
        inputSchema: recommendFilmsInputSchema,
    },
    handler: async (input) => {
        // input has already been validated against recommendFilmsInputSchema
        const { property_type, goals, application, vlt_preference, budget_level, install_location, sun_exposure, orientation, } = input;
        ...
        const recs = scoreFilms(films, {
            property_type,
            goals,
        });
        // scoreFilms may return brand-level suggestions; expand to concrete products
        const enriched = expandToProducts(recs, films, goals, {
            property_type,
            install_location,
            vlt_preference,
            budget_level,
        ...
        const criteriaSummary = (() => {
            ...
            if (vlt_preference)
                bits.push(`${prettyCase(vlt_preference)} look`);
            if (budget_level)
                bits.push(`${prettyCase(budget_level)} budget`);
            if (install_location)
                bits.push(`${prettyCase(install_location)} install`);
            if (sun_exposure)
                bits.push(`${prettyCase(sun_exposure)} sun`);
            if (orientation)
                bits.push(`${prettyCase(orientation)} facing`);
            return bits.join(" • ");
        })();
        return {
            content: [
